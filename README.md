...
Bước seed chạy một lần lúc khởi động, sau khi migration đã áp: 30 tài khoản (mật khẩu chung
`Demo#12345`), 4 project kèm label và milestone, ticket, sự cố và phản hồi rải theo project —
và **quyền project** cho cả dàn tài khoản đó, nếu không thì mọi tài khoản trừ admin đều không
vào được project nào.

| Vai trò  | Ví dụ tài khoản     |
| :-------- | :---------------------- |
| admin     | `bao.long@demo.local` |
| responder | `thu.ha@demo.local`   |
| support   | `gia.bao@demo.local`  |
| customer  | `anh.tu@demo.local`   |
...
[`infra/postman/incident-tracker.postman_collection.json`](infra/postman/incident-tracker.postman_collection.json)
