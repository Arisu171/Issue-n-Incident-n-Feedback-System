...
| `JWT__ISSUER` | | `gitissues-api` | Claim `iss` |
| `JWT__AUDIENCE` | | `gitissues-web` | Claim `aud` |
| `JWT__EXPIRYMINUTES` | | `15` | Hạn access token (ADR-001) |
| `SEED__APPLYMIGRATIONS` | | `true` | Chạy migration khi khởi động |
| `SEED__ADMINEMAIL` | | `admin@gitissues.local` | Admin bootstrap |
| `SEED__ADMINPASSWORD` | | *(trống)* | Bỏ trống thì **không** tạo admin |
| `SEED__SYSTEMEMAIL` | | *(trống)* | Tài khoản vai trò `system`. **Không có mặc định trong mã** |
| `SEED__SYSTEMPASSWORD` | | *(trống)* | Thiếu một trong hai thì admin giữ tạm vai trò `system` |
