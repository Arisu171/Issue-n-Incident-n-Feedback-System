...
public sealed class SeedOptions
{
    ...
    public string AdminEmail { get; set; } = "admin@gitissues.local";
    ...
    /// <summary>
    /// Email tài khoản mang vai trò <c>system</c>.
    ///
    /// <b>Cố ý KHÔNG có giá trị mặc định.</b> Khác với admin, danh tính của tài khoản này chỉ
    /// tồn tại trong cấu hình triển khai (<c>SEED__SYSTEMEMAIL</c>). Một mặc định nằm trong mã
    /// nguồn là địa chỉ ai đọc repo cũng biết, mà đây lại là tài khoản duy nhất chạm được cấu
    /// hình nền của toàn hệ thống.
    /// </summary>
    public string? SystemEmail { get; set; }

    /// <summary>
    /// Mật khẩu tài khoản <c>system</c>. Thiếu email hoặc mật khẩu thì bỏ qua bước tạo.
    ///
    /// <b>Chừng nào chưa cấu hình đủ hai giá trị</b>, tài khoản admin bootstrap giữ tạm vai trò
    /// <c>system</c>: gỡ ra khi chưa có ai khác cầm là mất vĩnh viễn đường vào cấu hình nền, vì
    /// <c>admin</c> ở rank 100 không tự gán lại được vai trò rank 120.
    /// </summary>
    public string? SystemPassword { get; set; }

    /// <summary>Tên hiển thị — không phải danh tính đăng nhập nên có mặc định là được.</summary>
    public string SystemDisplayName { get; set; } = "Tài khoản hệ thống";

    /// <summary>Đã cấu hình đủ để dựng tài khoản system riêng hay chưa.</summary>
    public bool HasSystemAccount =>
        !string.IsNullOrWhiteSpace(SystemEmail) && !string.IsNullOrWhiteSpace(SystemPassword);
...
public static class DatabaseInitializer
{
    public static async Task InitializeAsync(IServiceProvider services, CancellationToken ct = default)
    {
        ...
        {
            await EnsureSearchPathSchemaAsync(db, logger, ct);
        ...
        await SeedSystemAsync(db, hasher, options, logger, ct);

        // Architecture v3.1 — project mặc định, 9 label GitHub, Issue Type, SLA policy.
        await TicketSeeder.SeedAsync(db, logger, ct);

        // Dữ liệu trình diễn KHÔNG chạy ở đây nữa — xem DemoDataHostedService.
        //
        // Nó từng chạy ngay tại chỗ này, tức trước `app.Run()`, nên tiến trình chưa mở cổng
        // trong suốt thời gian seed. Trên máy nhanh thì không ai để ý; trên hạ tầng chậm với
        // database ở xa thì nền tảng quét cổng không thấy gì và giết tiến trình giữa chừng —
        // để lại một database seed dở và một service 502. Migration và dữ liệu tham chiếu vẫn
        // phải chặn vì API không phục vụ nổi khi thiếu schema; dữ liệu trình diễn thì không.
    }

    /// <summary>
    /// Tạo sẵn schema ghi trong <c>Search Path</c> của chuỗi kết nối, nếu nó chưa tồn tại.
    ///
    /// EF không tạo hộ: bảng lịch sử migration được ghi thẳng vào schema đó, nên schema thiếu
    /// là deploy chết ngay ở lệnh đầu tiên. Trước đây đây là một bước làm tay dễ quên — và khi
    /// quên thì thông điệp lỗi không hề nhắc tới schema.
    /// </summary>
    private static async Task EnsureSearchPathSchemaAsync(AppDbContext db, ILogger logger, CancellationToken ct)
    {
        var schema = new Npgsql.NpgsqlConnectionStringBuilder(db.Database.GetConnectionString())
        ...
        // Tên schema đến từ chuỗi kết nối của chính hệ thống, nhưng vẫn chặn ký tự lạ: nó đi vào
        // câu lệnh DDL mà DDL không nhận tham số.
        if (!System.Text.RegularExpressions.Regex.IsMatch(schema, "^[A-Za-z_][A-Za-z0-9_]*$"))
        {
            throw new InvalidOperationException(
                $"Search Path '{schema}' không phải một tên schema hợp lệ.");
        }

        await db.Database.ExecuteSqlRawAsync($"CREATE SCHEMA IF NOT EXISTS \"{schema}\"", ct);
        logger.LogInformation("Schema '{Schema}' đã sẵn sàng.", schema);
    }
        ...
        SeedOptions options, ILogger logger, CancellationToken ct)
    {
        ...
        // `system` là vai trò của một tài khoản RIÊNG (xem SeedSystemAsync). Admin chỉ mang nó
        // khi chưa cấu hình tài khoản đó — vì cấu hình nền (SLA, loại issue) chỉ `system` chạm
        // được, mà `admin` ở rank 100 không tự gán lại được vai trò rank 120. Không có ai cầm
        // thì mất vĩnh viễn; nên đây là dự phòng, không phải thiết kế.
        var adminAlsoSystem = !options.HasSystemAccount;
        ...
        if (existing is not null)
        {
            ...
            var hasSystem = existing.UserRoles.Any(ur => ur.RoleId == systemRole.Id);

            if (adminAlsoSystem && !hasSystem)
            {
                existing.UserRoles.Add(new UserRole { UserId = existing.Id, RoleId = systemRole.Id });
                repaired.Add("gán role system (chưa có tài khoản system riêng)");
            }
            else if (!adminAlsoSystem && hasSystem)
            {
                // Đã có tài khoản system riêng thì admin không giữ vai trò đó nữa. An toàn vì
                // SeedSystemAsync chạy ngay sau đây và luôn bảo đảm có người cầm.
                existing.UserRoles.Remove(existing.UserRoles.First(ur => ur.RoleId == systemRole.Id));
                repaired.Add("gỡ role system khỏi admin");
            }
    ...
    /// <summary>
    /// Tài khoản mang vai trò <c>system</c> — tách hẳn khỏi tài khoản admin.
    ///
    /// <b>Vì sao tách.</b> Cấu hình nền (ngưỡng SLA, loại vấn đề) là thứ hiếm khi đụng tới và
    /// đụng sai thì ảnh hưởng toàn hệ thống. Để chung một tài khoản với công việc quản trị
    /// hằng ngày nghĩa là mọi phiên đăng nhập thường lệ đều mang sẵn quyền đó.
    ///
    /// Tách được là nhờ ràng buộc rank: <c>admin</c> ở cấp 100 không gán nổi vai trò cấp 120
    /// cho bất kỳ ai, kể cả chính mình (RbacService — "role ngang hoặc cao hơn cấp của bạn").
    /// Nếu không có ràng buộc đó thì việc tách chỉ là hình thức.
    ///
    /// Tự chữa giống admin: mất vai trò hoặc bị khoá thì khôi phục ở lần khởi động sau, vì đây
    /// là lối thoát hiểm duy nhất cho cấu hình nền.
    /// </summary>
    private static async Task SeedSystemAsync(AppDbContext db, IPasswordHasher<User> hasher,
        SeedOptions options, ILogger logger, CancellationToken ct)
    {
        if (!options.HasSystemAccount)
        {
            logger.LogWarning(
                "Bỏ qua seed tài khoản system vì thiếu SEED__SYSTEMEMAIL hoặc SEED__SYSTEMPASSWORD. "
                + "Trong lúc đó tài khoản admin giữ tạm vai trò 'system'.");
            return;
        }

        var email = IdentityService.NormalizeEmail(options.SystemEmail!);
        var systemRole = await db.Roles.FirstAsync(r => r.Name == Permissions.SystemRole, ct);
        ...
        if (existing is not null)
        {
            ...
            {
                await db.SaveChangesAsync(ct);
                logger.LogWarning("Tài khoản system '{Email}' đã mất quyền; khôi phục: {Actions}.",
                    email, string.Join(", ", repaired));
            }
        ...
        var account = new User
        {
            ...
            Email = email,
            Login = await LoginNames.EnsureUniqueAsync(db, LoginNames.FromEmail(email), ct),
            DisplayName = options.SystemDisplayName,
            IsActive = true,
            CreatedAt = DateTimeOffset.UtcNow
        };
        // `!` an toàn: HasSystemAccount ở đầu hàm đã loại trường hợp null/rỗng.
        account.PasswordHash = hasher.HashPassword(account, options.SystemPassword!);

        // Chỉ `system`, không kèm `admin`: hai tài khoản, hai phạm vi.
        account.UserRoles.Add(new UserRole { UserId = account.Id, RoleId = systemRole.Id });

        db.Users.Add(account);
        await db.SaveChangesAsync(ct);

        logger.LogInformation("Đã tạo tài khoản system '{Email}'.", email);
    }
}
