...
public sealed class OrganizationTests
{
    ...
    public async Task Issue_type_is_org_level_one_per_ticket_and_disabled_type_is_kept_on_old_tickets()
    {
        ...
        // Loại vấn đề là cấu hình nền: `admin` không chạm được, chỉ `system`.
        var system = await _fx.SystemAsync();
        var created = await ReadJsonAsync(await system.PostAsJsonAsync("/api/issue-types", new { name = typeName, color = "purple" }, ApiFactory.Json));
        ...
        var id = S(created, "id");
        (await system.PatchAsJsonAsync($"/api/issue-types/{id}", new { name = typeName, isEnabled = false }, ApiFactory.Json)).EnsureSuccessStatusCode();
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await system.DeleteAsync($"/api/issue-types/{id}")).StatusCode);
    }
