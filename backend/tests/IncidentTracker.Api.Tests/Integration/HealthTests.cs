...
public class HealthTests
{
    ...
    /// <summary>
    /// Web nằm sau cổng đăng nhập thì phải báo Unhealthy, không được báo xanh.
    ///
    /// <b>Ca này đến từ một lần deploy thật.</b> Web bị bảo vệ bằng SSO của nhà cung cấp nên
    /// <c>/healthz</c> trả 302, và điểm đến cuối cùng là trang đăng nhập — một trang 200 hoàn
    /// toàn khoẻ mạnh. HttpClient mặc định bám redirect, nên health check thấy 200 và báo web
    /// XANH trong khi không người dùng nào mở nổi web.
    ///
    /// Niềm tin sai còn tệ hơn không có phép kiểm: nó khiến người vận hành đi tìm lỗi ở chỗ khác.
    /// </summary>
    [Fact]
    public async Task Web_nam_sau_cong_dang_nhap_thi_bao_Unhealthy()
    {
        using var stubWeb = new StubWebService(redirectToLogin: true);
        using var factory = _fx.Factory.WithWebHostBuilder(builder =>
            builder.UseSetting("HealthChecks:WebUrl", stubWeb.HealthUrl));
        ...
        // 503: /api/health/system trả mã lỗi khi có thành phần hỏng.
        ...
        Assert.Equal("Unhealthy", byName["web"]);

        // api và db vẫn khoẻ: chỉ web hỏng, và báo cáo phải chỉ đúng thủ phạm.
        Assert.Equal("Healthy", byName["api"]);
        Assert.Equal("Healthy", byName["db"]);
    }
