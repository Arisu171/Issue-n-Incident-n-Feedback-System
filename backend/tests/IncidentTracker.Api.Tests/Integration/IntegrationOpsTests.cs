...
public sealed class IntegrationOpsTests
{
    ...
    public async Task Sla_scheduler_writes_breach_and_escalation_once_and_notifies()
    {
        ...
        // Ngưỡng SLA là cấu hình nền: `admin` không chạm được, chỉ `system`.
        var system = await _fx.SystemAsync();
        (await system.PutAsJsonAsync("/api/sla-policies/P0", new { responseTimeMinutes = 0, resolutionTimeMinutes = 60, escalateAfterMinutes = 0, isActive = true }, ApiFactory.Json)).EnsureSuccessStatusCode();
