...
**Bốn tab chuẩn bị sẵn.** Ba tài khoản demo dùng mật khẩu chung `Demo#12345`; tab 1 là admin
bootstrap, mật khẩu lấy từ `SEED_ADMIN_PASSWORD` của chính môi trường đang chạy.

| Tab | Tài khoản | Vai trò | Phạm vi project |
|---|---|---|---|
| 1 | `admin@gitissues.local` | admin | toàn hệ thống |
| 2 | `gia.bao@demo.local` | support | Demo Alpha + Demo Beta |
| 3 | `thu.ha@demo.local` | responder | Demo Alpha + Demo Beta |
| 4 | `anh.tu@demo.local` | customer | Demo Alpha + Demo Beta |

Cần một tài khoản **chỉ thuộc một project** để cho thấy việc cách ly (phút 3–4): dùng
`minh.anh@demo.local` (manager, chỉ Demo Alpha) hoặc `khanh.chi@demo.local` (responder, chỉ
Demo Beta).
