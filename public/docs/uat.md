...
Tài khoản demo: mật khẩu chung `Demo#12345` — `bao.long@demo.local` (manager, cả hai project),
`minh.anh@demo.local` (manager, chỉ Demo Alpha), `thu.ha@demo.local` (responder),
`gia.bao@demo.local` (support), `anh.tu@demo.local` (customer). Admin là tài khoản bootstrap
`admin@gitissues.local`, mật khẩu lấy từ `SEED_ADMIN_PASSWORD` — không nằm trong dữ liệu trình diễn.
