...
| `postman/` | Collection kiểm chứng sức khỏe / vòng đời (`incident-tracker`) và hardening tuần 3 (`week-3-auth-rbac`) — CI chạy cả hai |
...
`postman/incident-tracker.postman_collection.json` — 9 request, 31 assertion. Import vào
Postman rồi đặt biến `adminPassword`, hoặc chạy bằng newman:

```bash
docker run --rm --network host -v "$PWD/infra/postman:/etc/newman"   postman/newman:alpine run /etc/newman/incident-tracker.postman_collection.json   --env-var "adminPassword=$(grep '^SEED_ADMIN_PASSWORD=' .env | cut -d= -f2-)"
```
