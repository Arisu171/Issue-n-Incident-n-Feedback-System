...
export function useSessionKeepAlive(): void {
  useEffect(() => {
    ...
    // `passive` để không chặn cuộn; `pointerdown`/`keydown` đủ đại diện cho thao tác thật,
    // không dùng `mousemove` vì chuột nhích một chút không có nghĩa là đang làm việc.
