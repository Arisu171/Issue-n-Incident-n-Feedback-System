    ...
    <>
      ...
      <div className="settings-grid">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          ...
          {flag('blankIssuesEnabled', 'Allow blank issues', tr('Tắt = khách hàng bắt buộc chọn template (config.yml blank_issues_enabled).'))}
          {flag('strictClosePolicy', tr('Strict close policy — chặn đóng khi còn sub-issue mở'), tr('Mặc định tắt (giống GitHub: chỉ cảnh báo). Bật → API trả 409 (BR-REL-02).'))}
          {flag('autoReopenOnCustomerComment', tr('Tự mở lại khi khách hàng bình luận'), tr('Mặc định tắt (Product Owner xác nhận: không tự mở lại).'))}
          {flag('customersSeeOnlyOwn', tr('Khách hàng chỉ thấy ticket của mình'), tr('Bật để cô lập dữ liệu giữa các khách hàng (BR-SEC-01).'))}
  ...
  return (
    <>
      ...
      <div style={{ fontSize: 12, color: 'color-mix(in srgb, var(--color-text) 60%, transparent)', marginTop: 'var(--space-2)' }}>
        {tr('Ký bằng')} <code>X-Hub-Signature-256</code> · timeout 10 giây · thử lại 1m / 5m / 30m / 2h / 6h rồi vào DLQ.
      </div>
      {deliveries && (
