...
import { isUncategorized, useActiveProject } from '@/lib/projectScope';
...
export function ProjectNav() {
  ...
  // Mục chưa phân loại chỉ chứa được sự cố và phản hồi — hai bảng duy nhất cho `project_id`
  // null. Bày ra tab Issues/Labels/Settings ở đó là mời người dùng bấm vào một màn hình chắc
  // chắn lỗi.
  const virtual = isUncategorized(slug);

  const items = [
    ...(virtual ? [] : [{ key: 'issues', href: `/projects/${slug}/issues`, label: 'Issues', count: project?.openTickets, match: (p: string) => p.startsWith(`/projects/${slug}/issues`) }]),
    ...
    ...(virtual ? [] : [{ key: 'labels', href: `/projects/${slug}/labels`, label: 'Labels', match: (p: string) => p === `/projects/${slug}/labels` }]),
    ...
    ...(session.can('ticket.write') && !virtual
