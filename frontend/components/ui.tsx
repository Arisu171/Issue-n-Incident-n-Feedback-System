 ...
 */
function useAnchoredBox(ref: React.RefObject<HTMLElement | null>, open: boolean): AnchoredBox | undefined {
  ...
  const place = useCallback(() => {
    ...
    setBox(flip
      ? { left: r.left, width: r.width, maxHeight: room, bottom: window.innerHeight - r.top + gap }
      : { left: r.left, width: r.width, maxHeight: room, top: r.bottom + gap });
  }, [ref]);
  ...
  return (
    <div className={`custom-select ${inline || chip ? 'inline' : ''} ${className}`.trim()} ref={ref} style={style}>
      <button
        type="button"
        id={id}
        className={`custom-select-trigger ${chip ? 'btn-chip' : ''} ${open ? 'open' : ''}`.trim()}
